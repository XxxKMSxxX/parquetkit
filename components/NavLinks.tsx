...
export function NavLinks() {
  ...
  return (
    <nav
      aria-label="Tools"
      className="-ml-2 flex flex-wrap items-center gap-x-1 gap-y-1 text-sm text-neutral-600 sm:ml-0 dark:text-neutral-400"
    >
      {links.map((link) => {
        const active =
      ...
      <span
        aria-hidden="true"
        className="mx-1 h-4 w-px bg-neutral-200 sm:mx-2 dark:bg-neutral-800"
      />
      <a
        href="https://github.com/XxxKMSxxX/parquetkit"
        rel="noopener"
        aria-label="GitHub"
        className="flex items-center gap-1.5 rounded-md px-2 py-1.5 transition-colors sm:px-2.5 hover:bg-neutral-100 hover:text-neutral-900 dark:hover:bg-neutral-800/60 dark:hover:text-neutral-100"
      >
        <svg
          viewBox="0 0 16 16"
